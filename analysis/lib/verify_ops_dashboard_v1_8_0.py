"""Synthetic verification for ops_dashboard v1.8.0.

No database is reachable from here, so the changed cells are executed against synthetic
feeds shaped to carry every case this release changes behaviour on:
  ...
  * an employee with ON-CALL hours straddling 40h      (threshold excludes them; labour
                                                        cost keeps them)
...
Cells executed (v1.8.0 indices): 4.3 (=20), 13.5 (=66), 14 (=68), 15.5 (=72), 15.6 (=74),
17.5 (=80).

Run from the repository root:

    python analysis/lib/verify_ops_dashboard_v1_8_0.py
...
NB = 'analysis/ops_dashboard_2026-08-14_v1_8_0.ipynb'
OUT = os.path.join(tempfile.gettempdir(), 'ops_dashboard_verify_v180')
