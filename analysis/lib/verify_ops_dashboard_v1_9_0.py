"""Synthetic verification for ops_dashboard v1.9.0.

The v1.8.1 harness (43 checks) re-run against v1.9.0's shifted cell indices, plus the three v1.9.0
changes:

  METRO VOCABULARY (Cell 4.2)
    * 'R16 Spring' reaches Houston and 'R03 Hot Springs' does NOT   [the whole-word fix]
    * 'R12 Ft. Worth' / 'R16 N. Houston' / 'R16 H3S' / 'R14 San Antonio WH' all land
    * an excluded warehouse stays out however well it matches

  PAYROLL PCT ROSTER (Cell 13.4, new)
  ...
  CENSUS ON THE PAYROLL DENOMINATOR (Cell 13.5)
    ...
    * the RETAINED ratio is bit-for-bit what v1.8.1 produced
...
...and everything the v1.8.1 harness covered: windowed active denominator, pooled lost-equipment
recovery, the on-call OT threshold, the OT feed guard, per-head OT pooling, the OT calibration,
spike surveillance, the full renderer and MMM-YY axis labels.

Cells executed (v1.9.0 indices): 4.2 (=18), 4.3 (=20), 13.4 (=66), 13.5 (=68), 14 (=70),
15.5 (=74), 15.6 (=76), 17.5 (=84), 22 (=94), 18 (=86), 21 (=92), 23 (=96), 16.5 (=80).

Cells 21 and 23 were never under test before v1.9.0, and they are the two that name columns
as strings — a dictionary entry or an executive-summary table can go stale against a renamed
column with no error anywhere. Both are now executed and their narrative asserted.

Run from the repository root:

    python analysis/lib/verify_ops_dashboard_v1_9_0.py
...
NB = 'analysis/ops_dashboard_2026-08-14_v1_9_0.ipynb'
OUT = os.path.join(tempfile.gettempdir(), 'ops_dashboard_verify_v190')
...
G = dict(
    ...
    ROUTING_CHANGE_DATE='2026-02-01', RUN_DATE='2026-08-19', NB_VERSION='1.9.0',
...
# ═══════════════════════════════════════════════════════════════════════════════
print('\n=== Cell 4.2 — assign_metro: whole-word matching [v1.9.0] ====================')
exec(CELL[18], G)
...
_CASES = [
    ...
    ('R12 Ft. Worth',       'DFW',         'the spelling v1.8.1 had no pattern for'),
    ...
    ('R16 H3S',             'Houston',     'the Houston-region code, unmatched in v1.8.1'),
...
# ═══════════════════════════════════════════════════════════════════════════════
print('\n=== Cell 13.4 — payroll PCT roster (the hired establishment) [v1.9.0] ========')
_buf134 = io.StringIO()
...
check('v1.9.0 RESTATEMENT' in _out135,
      'the run log prints the v1.9.0 restatement table on every run')
...
# ═══════════════════════════════════════════════════════════════════════════════
print('\n=== Cell 17.5 — scorecard & technician detail, every level [v1.9.0] ==========')
...
check('census_per_pct_on_payroll' in set(SC['Metro']['metric']),
      'the v1.9.0 census headline is on the scorecard')
...
check({'vp_value', 'vps_in_metric'} <= set(SC['VP'].columns),
      'the pre-v1.9.0 VP_Scorecard column names survive as aliases')
...
# ═══════════════════════════════════════════════════════════════════════════════
print('\n=== Cell 22 — recommendation documents, VP and Metro [v1.9.0] ================')
_BLK = []
...
check(G['vp_rec_blocks'] and set(G['vp_rec_blocks']) == set(_VPS),
      'the v1.6.0 vp_rec_blocks alias still resolves for Cell 23')
...
G['dash_tb_wh'] = _tb
G['DASH_PDF_NAME'] = 'verify_pages_v190.pdf'
with contextlib.redirect_stdout(io.StringIO()):
    exec(CELL[86], G)
...
check('SUPERSEDED' in _abul,
      'and marks the 2026-08-17 confirmation superseded rather than deleting it')
