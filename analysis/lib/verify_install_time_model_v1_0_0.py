"""Synthetic verification for install_time_model v1.0.0.

The model cells are executed against a synthetic order-line feed with KNOWN ground truth
(base visit time 12 min; concentrator 25, bed 40, wheelchair 15, tubing 0 min/unit), plus
every planted defect the QC/anomaly machinery exists for:
  ...
  * a zero-install-time product                           (NNLS boundary -> R6 flag)
  * 20 tickets ~4x slower than their product mix warrants (R9-slow)

Cells executed (v1.0.0 indices): 10 (run_query guard, no connection), 16, 18, 20, 22,
24, 26, 28, 30, 32, 34.

Run from the repository root:

    python analysis/lib/verify_install_time_model_v1_0_0.py
...
NB = 'analysis/install_time_model_2026-08-18_v1_0_0.ipynb'
OUT = os.path.join(tempfile.gettempdir(), 'install_time_verify_v100')
...
G = dict(
    ...
    OTHER_LABEL='OTHER (below support floor)', N_BOOT=60, BOOT_SEED=20260818,
...
check(est['TUBING'] <= 1.0, f"zero-cost product sits at the NNLS boundary ({est['TUBING']:.2f})")
check(G['R2'] > 0.5, f"fit explains the synthetic variance (R2={G['R2']:.3f})")
...
check(0.9 <= _ok['ratio'].median() <= 1.1,
      f"median actual/expected centred ({_ok['ratio'].median():.2f})")
exec(CELL[30], G)
...
for c in ('R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R9-slow'):
